...
class BorrowingController extends Controller
{
    ...
    public function borrow(Request $request)
    {
        ...
        $borrowing->borrowed_at =  $this->getDate();
        $borrowing->must_be_returned_at =  $this->getDate()->addDay(15);
    ...
    {
        $borrowing1 = Borrowing::where('must_be_returned_at','<',$this->getDate()->addDays(1));
        $borrowing2 = Borrowing::where('must_be_returned_at','<',$this->getDate()->addDays(2));
        $borrowings = Borrowing::where('must_be_returned_at','<',$this->getDate())
                                ->union($borrowing1)->union($borrowing2)->get();
        foreach($borrowings as $borrowing)
        {
            $user = User::where('person_id','=',$borrowing->user_id)->distinct()->first();
    ...
    public function destroy($nbr)
    {
        ...
        $borrowing_history->returned_at	= $this->getDate();
